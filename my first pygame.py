...
def RPSGAME():
    global totalgames , totalpoints , wins , loses 
    flag = 0
    com = 0
    user = 0
...
def check_win():
    ...
    if board[0] == board[1] == board[2] != '-' or board[3] == board[4] == board[5] != '-' or board[6] == board[7] == \
        ...
        print('GAME OVER')
        game_on = False
    elif board[0] == board[3] == board[6] != '-' or board[1] == board[4] == board[7] != '-' or board[2] == board[5] == \
            board[8] != '-':
        print('GAME OVER')
        game_on = False
...
def check_num_range():
    ...
    if num in range(lrange, urange + 1):
        print("YOU WIN NUM IS IN THE RANGE")
        wins+=1
        totalgames+=1
        totalpoints+=2
    else:
...
def GAMEPLAY():
    while True:
        ...
        select_number()
        check_num_range()
        chose = input('do you want to continue(1=yes,2=no)..')
        if chose == '2':
            break
    ...
    while run:
        ...
        if alien.visible == True:
            if man.hitbox[1] < alien.hitbox[1] + alien.hitbox[3] and man.hitbox[1] + man.hitbox[3] > alien.hitbox[1]:
                if man.hitbox[0] + man.hitbox[2] > alien.hitbox[0] and man.hitbox[0] < alien.hitbox[0] + alien.hitbox[
                    2]:
                    man.hit()
                    score -= 5
        ...
        if keys[pygame.K_SPACE] and shoottime == 0 and alien.visible:
            ...
            if len(bullets) < 6:
                bullets.append(
                    bullet(round(man.x + man.width // 2), round(man.y + man.height // 2), 5, (0, 0, 0), facing))
...
def intro():
    ...
    try:
        cnx = mysql.connector.connect(host='localhost', user='root', password='tanmaythetanwar13', database='pygame')
        cursor = cnx.cursor()
        cursor.execute("create table scores (NAME varchar(20),wins int,loses int,totalpoints int, Totalgames int)")
        print('table created')
    except:
        print('Error in connection/ creating table / table already exists ...')


def update_table():
    global a, wins, loses, totalpoints, totalgames

    import mysql.connector
    try:
        cnx = mysql.connector.connect(host='localhost', user='root', password='tanmaythetanwar13', database='pygame')
        cursor = cnx.cursor()
        cursor.execute("insert into scores values(%s,%s,%s,%s,%s)", (a, wins, loses, totalgames, totalpoints))
        cnx.commit()
        print('record added')
    except:
        cnx = mysql.connector.connect(host='localhost', user='root', password='tanmaythetanwar13', database='pygame')
        cnx.rollback()  # undo the insert
        print("Record not added...")


def update_data():
    global a,wins,loses,totalpoints,totalgames, cnx
    import mysql.connector
    try:
        cnx = mysql.connector.connect(host='localhost', user='root', password='tanmaythetanwar13', database='pygame')
        cursor = cnx.cursor()
        sql= ' update scores set wins=%s,loses=%s,totalpoints=%s,totalgames=%s where NAME=%s '
        val=(wins,loses,totalpoints,totalgames,a)
...
def disp_alldata():
    ...
    try:
        ...
        print('NAME'.ljust(10), 'wins'.ljust(15), 'loses'.ljust(10), 'Totalpoints'.ljust(10), 'totalgames'.ljust(10))
        for x in results:
            print(str(x[0]).ljust(10), str(x[1]).ljust(15), str(x[2]).ljust(10), str(x[3]).ljust(10),
                  str(x[4]).ljust(10))
    except:
